...
test('packFileBoxChunk()', async t => {
  ...
  const packedStream = packFileBoxChunk(chunkStream, MessageSendFileStreamRequest)

  let name = ''
  let buffer = ''
  packedStream.on('data', (data: MessageSendFileStreamRequest) => {
...
test('packFileBoxChunk() <-> unpackFileBoxChunk()', async t => {
  ...
  const packedStream = packFileBoxChunk(stream, MessageSendFileStreamRequest)
