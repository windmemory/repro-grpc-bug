...
test('toMessageSendFileStreamRequestArgs()', async t => {
  ...
  t.equal(data.length, FILE_BOX_DATA.length, 'should get file box data')
})
...
test.only('toMessageSendFileStreamRequestArgs() <-> toMessageSendFileStreamRequest()', async t => {
  const FILE_BOX_NAME = 'image.jpg'
  const CONVERSATION_ID = 'conv_id'

  const fileBox = FileBox.fromFile(
    path.join(__dirname, './image.jpg')
  )
  const FILE_BOX_DATA = (await fileBox.toBuffer()).toString()
  ...
  t.equal(args.fileBox.name, FILE_BOX_NAME, 'should be same name')
  t.equal((await args.fileBox.toBuffer()).toString(), FILE_BOX_DATA, 'should be same content')
  t.equal(chunkCount, 248, 'chunk should be separate into multiple pieces')
})
